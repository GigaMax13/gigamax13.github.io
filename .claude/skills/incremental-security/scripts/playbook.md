...
# incremental-security playbook

## Original heading: incremental-security
...
/incremental-security PR 123 in packages/shared    # PR + folder
BATCH_SIZE=50; /incremental-security               # Custom batch (all-files only)
```

## Rules

1. **NEVER STOP BETWEEN STEPS** — no user output until step 5c completes.
2. Sub-skill output is intermediate data; only valid final output is the 5c status line.
3. **MUST write `$DEV_DIR/security.md`** before any user output — the file IS the deliverable.
4. Findings exist but `$DEV_DIR/security.md` not written = FAILED.
...
SECURITY_FILE="$DEV_DIR/security.md"
...
Output: `Security Audit — Project: $PROJECT_ID, Scope: $SCOPE, Folder: $FOLDER_FILTER (or "none")`
...
case "$SCOPE" in
    ...
    pr)
        if ! command -v gh &>/dev/null; then
            echo "Error: gh CLI required for PR scope."
            exit 1
        fi
...
if [ -f "$SECURITY_FILE" ]; then
    LAST_BATCH=$(grep -m1 '^<!-- batch: ' "$SECURITY_FILE" | sed 's/<!-- batch: \([0-9]*\).*/\1/')
    TOTAL_FILES=$(grep -m1 '^<!-- total_files: ' "$SECURITY_FILE" | sed 's/<!-- total_files: \([0-9]*\).*/\1/')
    FILES_AUDITED=$(grep -m1 '^<!-- files_audited: ' "$SECURITY_FILE" | sed 's/<!-- files_audited: \([0-9]*\).*/\1/')
else
    LAST_BATCH=0; TOTAL_FILES=$FILE_COUNT; FILES_AUDITED=0
fi

BATCH_NUM=$((LAST_BATCH + 1))
BATCH_FILES=$(echo "$ALL_FILES" | tail -n +$((FILES_AUDITED + 1)) | head -n "$BATCH_SIZE")
BATCH_COUNT=$(echo "$BATCH_FILES" | grep -c '^')
[ "$BATCH_COUNT" -eq 0 ] && exit 0
NEW_FILES_AUDITED=$((FILES_AUDITED + BATCH_COUNT))
```

Output: `Batch $BATCH_NUM: $BATCH_COUNT files ($((FILES_AUDITED + 1))-$NEW_FILES_AUDITED of $FILE_COUNT)`

### 5. Execute Audit and Write Report

#### 5a. Call Security Audit Skill

`/security-audit $PROJECT_ID, $BATCH_FILES`

Capture FINDINGS. Check for `==== NO ISSUES ====` to determine HAS_FINDINGS.

#### 5b. Write .dev/security.md (MANDATORY)

- No findings + no prior security.md: skip file creation
- No findings + prior security.md: update metadata only
- Findings exist:
...
if [ ! -f "$SECURITY_FILE" ]; then
    echo "# Incremental Security Audit: $PROJECT_ID ($SCOPE_DESC)" > "$SECURITY_FILE"
    echo "<!-- batch: 0 -->" >> "$SECURITY_FILE"
    echo "<!-- total_files: $TOTAL_FILES -->" >> "$SECURITY_FILE"
    echo "<!-- files_audited: 0 -->" >> "$SECURITY_FILE"
    echo "<!-- last_updated: $(date -u '+%Y-%m-%dT%H:%M:%SZ') -->" >> "$SECURITY_FILE"
fi

echo "### Batch $BATCH_NUM ($(date '+%Y-%m-%d %H:%M:%S'))" >> "$SECURITY_FILE"
# incremental-security playbook
...
sed -i.bak \
    ...
    -e "s/<!-- files_audited: [0-9]* -->/<!-- files_audited: $NEW_FILES_AUDITED -->/" \
...
`Progress: $NEW_FILES_AUDITED/$FILE_COUNT — $REMAINING remaining` (or `All files audited!`).
If any batch had findings: `Security audit failed: $ISSUE_COUNT issue(s)` — Exit 1.

## Exit Criteria

- [ ] **security.md written to disk** (findings + metadata updated) — PRIMARY DELIVERABLE
- [ ] PROJECT_ID auto-detected
- [ ] Scope parsed (all/branch/pr + optional folder filter)
- [ ] File list resolved via git commands per scope
- [ ] Folder filter applied as post-filter
- [ ] Batch resolved (up to BATCH_SIZE files per invocation)
- [ ] Security-audit skill called with PROJECT_ID and batch files
- [ ] No report file when no issues found
- [ ] Status line output AFTER file write

After status line: print `INCREMENTAL-SECURITY COMPLETE`
